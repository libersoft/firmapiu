...
class ConfigReader(object):
    ...
    def handler_request(self, name):
        self._logger.debug('pass request to _function for "%s"' % name)
        result = self._function(name)  # richiamo la funzione per le richieste
    ...
    def try_to_obtain(self, result, name, errmsg):
        ...
        result = self.handler_request(name)  # tento con l'_handler
        if result is not None:  # se l'_handler e' giusto
            return result

        self._logger.error(errmsg)  # stampo il messaggio di errore nel _logger
        return None
...
class ConfigFileReader(ConfigReader):
    ...
    def __init__(self, fileconfig, logger):
        assert isinstance(fileconfig, str)
        
        if not os.access(fileconfig, os.R_OK):
            raise AttributeError
        
        ConfigReader.__init__(self, logger)
        self.config = ConfigParser.ConfigParser()
        self.config.read([fileconfig])

    def get_ca_certificate_path(self):
        self._logger.debug('read ca certificate path from config file')
    ...
    def get_crl_path(self):
        self._logger.debug('read crl path path from config file')
    ...
    def get_smartcard_info_path(self):
        self._logger.debug('read smartcard library path from config file')
    ...
    def get_smartcard_driver_path(self):
        self._logger.debug('read smartcard driver path from config file')
    ...
    def get_engine_driver_path(self):
        self._logger.debug('read engine path from config file')
    ...
    def get_timestamp_server(self):
        self._logger.debug('read timestamp server from config file')
    ...
    def get_timestamp_username(self):
        self._logger.debug('read timestamp username from config file')
    ...
    def get_timestamp_password(self):
        self._logger.debug('read timestamp password from config file')
    ...
    def get_smartcard_pin(self):
        self._logger.debug('read smartcard pin from config file')
    ...
    def get_smartcard_puk(self):
        self._logger.debug('read smartcard puk from config file')
